...
export function TeamStatsSummary({
  ...
  return (
    <Card className="w-full bg-gradient-to-br from-purple-500/10 via-blue-500/10 to-transparent border-purple-500/20 hover:scale-[1.02] hover:shadow-xl transition-all duration-300">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {formLetters.map((letter, index) => (
                  <Badge
                    key={index}
                    variant={getFormBadgeVariant(letter) as any}
                    className="w-7 h-7 flex items-center justify-center p-0 text-xs font-bold"
                    title={getFormLabel(letter)}
                  >
                    {letter}
                  </Badge>
                ))}
      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            label="Győzelmek"
            value={wins.toString()}
            subValue={`${((wins / totalMatches) * 100).toFixed(0)}%`}
          />
          <StatCard
          ...
          <StatCard
            ...
            label="CSS Score"
            value={
              <Badge variant={getCSSBadgeVariant(cssScore) as any} className="text-base">
                {cssScore.toFixed(1)}
              </Badge>
            }
          />
